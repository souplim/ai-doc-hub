...
`uploader`는 이 워크스페이스에서 Module Federation `remote` 역할을 하는 프로젝트입니다.

## 개요

이 프로젝트는 `./Uploader` 모듈을 외부에 노출합니다.
`container` 프로젝트는 이 모듈을 `uploader/Uploader`로 가져와 화면에 렌더링합니다.
...
3. 브라우저에서 `container` 앱 확인

만약 `container`가 먼저 뜨고 `uploader` preview가 아직 준비되지 않았다면, `remoteEntry.js` 404가 날 수 있습니다.
